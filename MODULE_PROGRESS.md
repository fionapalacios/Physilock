...
Tracks progress against the module-based development lifecycle in `PROJECT_DOCUMENTATION.md` (see its "Development Lifecycle" table for the canonical module list). Check items off as they're built; update the module-level status line whenever a module's checklist changes meaningfully. Last synced: 2026-08-23.
...
**Not built**
- [ ] Forgot-password flow, Email verification screen — UI now exists (`ForgotPasswordScreen`/`VerifyEmailScreen`, ported 2026-08-23, see above) but is unwired in `NavGraph` and has no real backend service behind it yet (no reset-code/verification-code service). The existing "Forgot password?" link in `AuthScreen.kt` still goes to a no-op.
- [ ] Mode-picker screen (Student/Work, chosen post-onboarding per [[project_auth_screens]]'s confirmed flow) — `SettingsScreen` has a mode toggle, but the dedicated post-login picker screen still doesn't exist. Teammate's `StudentModeScreen.kt`/`WorkModeScreen.kt` are still confirmed empty stubs (re-checked 2026-08-23) — no source to port from yet.
...
- [ ] Focus Mode screens (Module 6) — `FocusModeScreen`/`EndFocusSessionSheet` ported 2026-08-23 to `ui/focus/`, but unwired and UI-only; no session/timer/app-blocking backend exists (see Module 6 below).
